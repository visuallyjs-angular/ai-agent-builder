...
/**
 * replace the given placeholder with a new node, defined by `type` and `payload`, then connect the new node with the original parent. If the original parent vertex is a decision vertex, also ensure that it has at least one placeholder child. Run all of those operations in a transaction so they can be undone/redone as one unit.
 * @param placeholder
 * @param type
 * @param payload
 * @param model
...
/**
 * Deletes a condition from a decision node
 * @param obj
 * @param conditionId
 * @param model
 */
...
/**
 * Adds a condition to a decision node
 * @param obj
 * @param model
 */
...
/**
 * Sets the action used by a trigger
 * @param obj
 * @param model
 * @param trigger
 */
